...
# 2. Functions in Package

## 2.1.Package Install

Function checks related packages **whether they are installed or not**. If a package
is not installed, function installs package or if a package is already installed
then function imports package:
...
days_diff_check <- function(days_diff) {
  if (days_diff > 30 | days_diff < 0){
    validation_text <- "Validation error is occured. Day 
                        differences must not be greater 
                        than 30 or lower than 0."
    stop(validation_text)
  } else {
...
```{r scrape_target_url1, eval=FALSE, class.source='.custom-font-size', echo=TRUE, tidy=TRUE}
...
```{r scrape_target_url2, eval=FALSE, class.source='.custom-font-size', echo=TRUE, tidy=TRUE}
tryCatch(
    {
      # Read the xml file.
...
```{r scrape_target_url3, eval=FALSE, class.source='.custom-font-size', echo=TRUE, tidy=TRUE}
}, error = function(e) {
...
```{r get_exchange_rates_with_date_range1, eval=FALSE, echo=TRUE}
...
```{r get_exchange_rates_with_date_range2, eval=FALSE, echo=TRUE}
 for (added_day in 0:days_diff) {
    ...
    if (is.null(exchange_rates)){
      print(paste("There is an error on date",
                  format(start_date + added_day, "%d.%m.%Y"), 
                  sep=" "))
      next
```
...
```{r get_exchange_rates_with_date_range3, eval=FALSE, echo=TRUE}
...
get_exchange_rates_with_single_date <- function(date) {
  ...
  if (is.null(exchange_rates)){
    print(paste("There is an error on date", format(date, "%d.%m.%Y"), 
                sep=" "))
  } else if (is.numeric(exchange_rates)) {
    print(paste("There is no data on date", format(date, "%d.%m.%Y"), 
                sep=" "))
  } else {
    return (exchange_rates)
  }
...
# date of holiday
# result1 <- 
#get_exchange_rates_with_single_date("04.12.2022")
...
# normal date range
# result2 <- 
#get_exchange_rates_with_date_range("27.12.2022", "29.12.2022")
# bigger than 30 days
# result2 <- 
#get_exchange_rates_with_date_range("27.10.2022", "29.12.2022")
# lower than 0 day
# result2 <- 
#get_exchange_rates_with_date_range("27.12.2022", "29.10.2022")
# invalidated date format
# result2 <- 
#get_exchange_rates_with_date_range("27-12-2022", "29.10.2022")
```
